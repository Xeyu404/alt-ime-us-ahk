...
2026-06-25 時点で、両派生元リポジトリに明示的な `LICENSE` ファイルを確認できなかったため、このリポジトリでは MIT License などのオープンソースライセンスを独自に付与していません。

ライセンスが明示されていないコードは、自由な再配布、改変、商用利用が自動的に許可されるものではありません。このリポジトリの公開、再配布、派生物の提供を行う場合は、派生元の権利者へ許可とライセンス条件を確認してください。

GitHub 上で Fork として表示するかどうかは、この利用許諾の代わりにはなりません。この文書は権利関係の状況を明示するためのものであり、利用許諾を与えるものではありません。
